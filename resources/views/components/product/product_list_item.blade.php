<div class="tp-product-list-item d-md-flex" style="align-items: stretch; height: 280px;">
    <div class="tp-product-list-thumb p-relative fix" style="flex: 0 0 280px; width: 280px; height: 280px; overflow: hidden;">
        <a href="{{ route('products.show', $product) }}" style="display: block; width: 100%; height: 100%;">
            <img src="{{ asset('storage/' . $product->main_image) }}" alt="{{ $product->name }}" style="width: 100%; height: 100%; object-fit: cover; object-position: center; border-radius: 4px;">
        </a>
        ...
        <div class="tp-product-action-2 tp-product-action-blackStyle">
            <div class="tp-product-action-item-2 d-flex flex-column">
                <button type="button" 
                    ...
                    <span class="tp-product-tooltip tp-product-tooltip-right">{{ __('header.quick_view') }}</span>
                </button>
                <button type="button" 
                    ...
                    <span class="tp-product-tooltip tp-product-tooltip-right">{{ __('header.add_to_wishlist') }}</span>
                </button>
                <button type="button" 
                    ...
                    <span class="tp-product-tooltip tp-product-tooltip-right">{{ __('header.add_to_compare') }}</span>
                </button>
            </div>
        </div>
    </div>
    <div class="tp-product-list-content">
        <div class="tp-product-content-2 pt-15">
            <div class="tp-product-tag-2">
                @if($product->category)
                    <a href="{{ route('products.index', ['filter[category_id]' => $product->category->id]) }}">
                        {{ $product->category->name }}{{ $product->brand ? ', ' : '' }}
                    </a>
                @endif
                @if($product->brand)
                    <a href="{{ route('products.index', ['filter[brand_id]' => $product->brand->id]) }}">
                        {{ $product->brand->name }}
                    </a>
                @endif
            </div>
